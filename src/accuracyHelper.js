...
export default class accuracyHelper {
    constructor() {
        ...
        this.realtime_accuracy_percetage = false
    ...
    calculateAccuracy() {
        if (this.track_accuracy && this.shortest_distance <= 0.1) { // Conditional
            this.realtime_accuracy_percetage = (0.1 - this.shortest_distance)  * 10
            console.log('accuracy calculated: ', this.realtime_accuracy_percetage)

            if (!this.mean_accuracy_percentage) {
                this.mean_accuracy_percentage = this.realtime_accuracy_percetage
            }

            this.mean_accuracy_percentage = (this.realtime_accuracy_percetage + this.mean_accuracy_percentage) / this.sample_no
            this.sample_no += 1
        }
        // console.log(this.realtime_accuracy_percetage)
    }
